...
- Every POST handler runs behind the idempotency middleware. The handler does not see the `Idempotency-Key`; the middleware ensures a *successful* handler run happens at most once per unique `(token, METHOD+PATH, key, body-hash)` in the 24-hour window and caches that response. Cache disposition for non-success responses is explicit, never derived from the status alone: a refusal that provably committed nothing (pre-dispatch validation, not-found) marks itself via `idempotency.MarkRefusalUnconsumed` (MCP tools: the typed pure/not-found error markers) and leaves the key usable with a corrected body; every **unmarked** 4xx is conservatively recorded and replayed like a committed conclusion — meristem has stateful refusals (xylem/signal budget exhaustion appends authoritative events before returning 409) which must replay, and conflict on a changed body, rather than re-execute under one key. 5xx responses are never recorded: an incomplete attempt must re-execute on retry.
