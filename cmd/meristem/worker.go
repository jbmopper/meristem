...
func runWorkerOnce(ctx context.Context, logger *slog.Logger, args []string) error {
	...
	fmt.Fprintf(os.Stdout, "worker --once: scanned=%d emitted=%d already_recorded=%d patience_escalations=%d patience_escalations_already_requested=%d patience_escalations_skipped_awaiting_human=%d convergence_candidates=%d convergence_verdicts=%d stale_inputs_skipped=%d accepts=%d retries=%d escalations=%d\n",
		result.Scanned,
