...
func (createdProjector) Apply(ctx context.Context, tx pgx.Tx, event domain.Event) error {
	...
	// bug case.
	_, err := tx.Exec(ctx, `
		INSERT INTO work_items (id, title, body, state, created_by, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $6)
		ON CONFLICT (id) DO NOTHING
	`, event.SubjectID, payload.Title, payload.Body, state, event.ActorTokenID, event.OccurredAt)
	return err
}
