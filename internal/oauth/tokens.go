...
func (s *TokenService) AuthenticateAccess(ctx context.Context, secret, expectedResource string) (domain.Token, error) {
	...
	if tok.IsRoot || tok.Source != domain.SourceAgent || tok.RevokedAt != nil || grantRevoked != nil || clientRevoked != nil || !now.Before(tokenExpiry) || !now.Before(grantExpiry) || resource != expectedResource || scopeErr != nil || scope != expectedScope {
		return domain.Token{}, ErrInvalidAccessToken
	}
	return tok, nil
}
