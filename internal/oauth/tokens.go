...
var ErrRefreshReuse = errors.New("oauth: refresh token reuse detected; grant revoked")
...
func (s *TokenService) ExchangeCode(ctx context.Context, in RedeemInput) (TokenPair, error) {
	...
	var codeID, clientID, redirectURI, challenge, scope, resource, authorityProfile string
	...
	_, _, err = s.writer.Append(ctx, tx, events.Spec{SubjectKind: domain.SubjectOAuthGrant, SubjectID: grantID, Kind: domain.EventOAuthGrantIssued, Source: domain.SourceSystem, ActorTokenID: &systemActor.ID, Payload: map[string]any{"payload_version": 1, "grant_id": grantID, "client_id": clientID, "actor_token_id": actorID, "authority_profile": authorityProfile, "scope": scope, "resource": resource, "access_token_id": accessID, "access_token_hash_b64": base64.StdEncoding.EncodeToString(accessHash), "access_expires_at_unix": now.Add(AccessTokenTTL).Unix(), "refresh_token_id": refreshID, "refresh_token_hash_b64": base64.StdEncoding.EncodeToString(refreshHash), "refresh_expires_at_unix": now.Add(RefreshTokenTTL).Unix(), "generation": 1}})
	if err != nil {
		return TokenPair{}, err
	}
	...
	return TokenPair{AccessToken: access, RefreshToken: refresh, TokenType: "Bearer", ExpiresIn: int(AccessTokenTTL.Seconds()), Scope: scope}, nil
}
...
func (s *TokenService) Refresh(ctx context.Context, secret, clientID string) (TokenPair, error) {
	...
	_, _, err = s.writer.Append(ctx, tx, events.Spec{SubjectKind: domain.SubjectOAuthGrant, SubjectID: grantID, Kind: domain.EventOAuthGrantRefreshed, Source: domain.SourceSystem, ActorTokenID: &systemActor.ID, Discriminator: tokenID, Payload: map[string]any{"payload_version": 1, "old_refresh_token_id": tokenID, "new_refresh_token_id": refreshID, "new_refresh_token_hash_b64": base64.StdEncoding.EncodeToString(refreshHash), "generation": generation + 1, "refresh_expires_at_unix": grantExpiry.Unix(), "access_token_id": accessID, "access_token_hash_b64": base64.StdEncoding.EncodeToString(accessHash), "access_expires_at_unix": now.Add(AccessTokenTTL).Unix()}})
	if err != nil {
		return TokenPair{}, err
	}
	if err := tx.Commit(ctx); err != nil {
